    ...
    fn render(
        ...
        for entry in self.players.iter() {
            ...
            if esp_settings.skeleton {
                ...
                // Create bone map and find head bone
                let bone_map = BoneMap::new();
                let bone_names = entry_model.bones.iter().map(|bone| bone.name.clone()).collect::<Vec<_>>();
                let head_bone_index = bone_map.find_bone_index(BodyPart::Head, &bone_names);
                
                if let Some(head_bone_index) = head_bone_index {
                    if let Some(head_state) = pawn_model.bone_states.get(head_bone_index) {                        if let (Some(head_position), Some(head_far)) = (
