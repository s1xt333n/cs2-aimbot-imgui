...
mod hotkey {
    ...
    pub fn render_button_key(
        ...
        let mut updated = false;        if optional {
            // Use neuomorphic styling for hotkey buttons
            let neuomorphic_style = crate::settings::get_neuomorphic_style();
            use crate::utils::neuomorphic::NeuomorphicUi;
            
            if ui.neuomorphic_button(&button_label, [size[0] - 35.0, size[1]], &neuomorphic_style) {
                ui.open_popup(label);
            }
            ...
            ui.disabled(key.is_none(), || {
                if ui.neuomorphic_button("X", [25.0, 0.0], &neuomorphic_style) {
                    updated = true;
                    *key = None;
                }
            });
            ...
            if ui.neuomorphic_button(&button_label, size, &neuomorphic_style) {
                ui.open_popup(label);
            }
        }
